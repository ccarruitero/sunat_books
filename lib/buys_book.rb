...
class BuysBook < Book
  ...
  def initialize(company, tickets, view, month, year)
    ...
    repeat(:all, dynamic: true) do
      canvas do
        bounding_box([270, 50], width: 800) do
          book_footer
        end
      end
    end
  ...
  def book_footer
    ...
    if @pages[page_number - 1] != nil
      ...
      arr << [letter1, bi_sum1, igv_sum1, BigDecimal(0).round(2), '0.00', 0.00, 0.00, non_taxable1, 0.00, 0.00, total_sum1]
    end
    arr << [letter, bi_sum, igv_sum, 0.00, 0.00, 0.00, 0.00, non_taxable, 0.00, 0.00, total_sum]
